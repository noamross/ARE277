# Code from 'http://www.macalester.edu/~kaplan/math135/pplane.r', by Daniel Kaplan
...
jacobianAtXY <- function(fun,x=NULL, y=NULL){
  ...
  foox <- foo(x+h,y);
  fooy <- foo(x,y+h);
  A <- (foox[1] - foo(x,y)[1])/h;
  B <- (fooy[1] - foo(x,y)[1])/h;
  C <- (foox[2] - foo(x,y)[2])/h;
  D <- (fooy[2] - foo(x,y)[2])/h;
...
showcontours <- function(fun,xlims, ylims, resol=50,add=F, colors=c('blue', 'red')) {
